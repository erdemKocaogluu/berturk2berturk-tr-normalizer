...
MAX_LENGTH = 128
PREFIX = ""
OUTPUT_DIR = "models/berturk2berturk-tr-normalizer"
CER_W, WER_W = 0.6, 0.4
CLEAN_EVAL = "data/clean_source/tatoeba_eval.txt"
N_EXAMPLES = 5


def build_model(base_model, tokenizer):
...
def parse_args():
    ...
    p.add_argument("--base_model", default=MODEL_NAME,
                   help="base model: an HF model id (plain encoder, built into an encoder-decoder from scratch), "
                        "or a local folder path to an already-built encoder-decoder checkpoint")
    p.add_argument("--skip_gold", action="store_true", help="skip the post-training gold evaluation")
    p.add_argument("--gold_beam", type=int, default=4, help="beam size for the gold evaluation")
    return p.parse_args()
...
def main():
    ...
    model = build_model(args.base_model, tokenizer)
    ...
    def compute_metrics(eval_pred):
        ...
        src_txt = [(s[len(PREFIX):] if PREFIX and s.startswith(PREFIX) else s).strip() for s in src_raw]
